...
function EditPost() {
  ...
  useEffect(() => {
    try {
      if (
        tabFromUrl &&
        tabFromUrl.startsWith("edit_post-") &&
        tabFromUrl.length > "edit_post-".length
      ) {
        const blogSlug = tabFromUrl.slice("edit_post-".length);

        const fetchPost = async () => {
          const res = await fetch(`/api/post/getpost?_id=${blogSlug}`);
          const data = await res.json();
          if (!res.ok) {
            console.log(data.error);
          } else {
            setPostData(data.message.post.posts[0]);
          }
        };
  ...
  const handlePostSubmit = async (e) => {
    ...
    try {
      const res = await fetch(
        `/api/post/editPost/${postData._id}/${currentUser.message.user._id}`,
        {
          method: "PUT",
          headers: {
    ...
    <div className="dark:glass-container md:w-2/4 p-3 min-h-screen mx-auto  bg-gray-100 dark:bg-[#131315] rounded-lg shadow-md">
      ...
      <form className="flex flex-col gap-4 p-6 " onSubmit={handlePostSubmit}>
        ...
        {postImageUploadError && (
          <p className="text-red-500">{postImageUploadError}</p>
        )}
        {updateMessage && <p className="text-green-500">{updateMessage}</p>}
        {postData.image && (
          <img src={postData.image} className="w-full h-72 object-cover" />
        )}
        <ReactQuill
          theme="snow"
